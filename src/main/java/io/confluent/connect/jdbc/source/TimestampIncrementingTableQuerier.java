...
public class TimestampIncrementingTableQuerier extends TableQuerier implements CriteriaValues {
    ...
    public void maybeStartQuery(Connection db) throws SQLException, ConnectException {
        ...
        log.trace("Set the committed offset: {}",
                committedOffset.getTimestampOffset(TimestampColumnTypeUtil.getDefault(timestampColumnType)));
    }
    ...
    public Object beginTimestampValue() {
        return offset.getTimestampOffset(TimestampColumnTypeUtil.getDefault(timestampColumnType));
    }

    @Override
    public Object endTimestampValue() throws SQLException {
        log.debug("fetching end value from " + tableId.tableName() + " using column " + timestampColumns.get(0) + " with respect to type " + timestampColumnType);
        if (timestampColumnType.equals(JdbcSourceConnectorConfig.TIMESTAMP_COLUMN_TYPE_BINARY)) {
            ExpressionBuilder builder = dialect.expressionBuilder()
                    .append("select  max(")
            ...
            PreparedStatement endVal = dialect.createPreparedStatement(stmt.getConnection(), builder.toString());
            log.debug("executing query: " + builder.toString());

            ResultSet result = endVal.executeQuery();
